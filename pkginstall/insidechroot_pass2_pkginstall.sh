        ...
        if [[ -f $PDP_SRC/$TAR_FILENAME ]]; then
            ...
            if [[ ! -d $DIRNAME ]]; then
                ...
                pushd $DIRNAME
                    ...
                    if [[ -f $PDP_PKGINSTALL/insidechroot_pass2_$PACKAGE.sh ]] ; then
                        if ! source $PDP_PKGINSTALL/insidechroot_pass2_$PACKAGE.sh ; then
...
for package in $@
do
    PKGInstall $package | tee $LOGS_DIR/pass2_$package.log
done
